	...
	public static class NioEchoClient implements  Runnable{
		...
		private void doConnect() throws IOException {
			if(this.socketChannel.connect(new InetSocketAddress(host, port))) {
				...
				SystemUtil.println("Successful to connect to the server.");
			}else {
		...
		private void doWrite(SocketChannel channel, String message) throws IOException {
			...
			SystemUtil.println("Write message to server:" + message);
		}
			...
			if(key.isValid()) {
				...
				if(key.isConnectable()) {
					if(sc.finishConnect()) {
						...
						SystemUtil.exitWithErrorMessage("Failed to connect to the server. ");
					}
				}
				if(key.isReadable()) {
